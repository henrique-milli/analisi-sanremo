...
def generate_pie_charts(count, title, filename):
    ...
    color_map = {truncate_label(label): colors[i % len(colors)] for i, label in enumerate(labels)}

    # Filter out zero values
    filtered_labels_m = [label for label, size in zip(truncated_labels, sizes_m) if size > 0]
    filtered_sizes_m = [size for size in sizes_m if size > 0]
    filtered_labels_f = [label for label, size in zip(truncated_labels, sizes_f) if size > 0]
    ...
    ax1.pie(filtered_sizes_m, labels=filtered_labels_m, startangle=140, labeldistance=1.1, textprops={'fontsize': 8}, colors=filtered_colors_m, autopct='%1.1f%%')
    ax1.set_title(f'{title} - Male')

    ax2.pie(filtered_sizes_f, labels=filtered_labels_f, startangle=140, labeldistance=1.1, textprops={'fontsize': 8}, colors=filtered_colors_f, autopct='%1.1f%%')
    ax2.set_title(f'{title} - Female')

    fig.legend(truncated_labels, loc='lower center', ncol=5)
...
def main():
    ...
    themes_df = generate_table(themes_count, themes_percentage, ['Theme', 'Count M', 'Percentage M', 'Count F', 'Percentage F', 'Positive Sentiment', 'Negative Sentiment'], include_sentiments=True)
    ...
    genre_df = generate_table(genre_count, genre_percentage, ['Genre', 'Count M', 'Percentage M', 'Count F', 'Percentage F'])
    ...
    sentiment_df = generate_table(sentiment_count, sentiment_percentage, ['Sentiment', 'Count M', 'Percentage M', 'Count F', 'Percentage F'])
